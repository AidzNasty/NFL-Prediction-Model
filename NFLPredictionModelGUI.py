...
def load_data():
    ...
    try:
        ...
        # Load ML predictions from Excel sheet "ML Prediction Model" (columns B-N to include ml_correct)
        ml_predictions = None
        try:
            ml_raw = pd.read_excel(EXCEL_FILE, sheet_name='ML Prediction Model', header=None)
            
            # Extract columns B through N (indices 1-13) to include ml_correct
            ml_data = ml_raw.iloc[:, 1:14].copy()
